...
RUN --mount=type=cache,id=s/92ca8a61-c1ba-421f-a389-d48ac7258c2d-next-cache,target=/app/.build/next/cache \
  mkdir -p /app/data \
  && npm run build \
  && node --input-type=module -e "import { createRequire } from 'node:module'; import { pathToFileURL } from 'node:url'; const standaloneRoot = '/app/.build/next/standalone/node_modules/'; const require = createRequire('/app/.build/next/standalone/package.json'); for (const pkg of ['@atjsh/llmlingua-2', '@huggingface/transformers', 'js-tiktoken']) { try { const resolved = require.resolve(pkg); if (!resolved.startsWith(standaloneRoot)) console.warn('[Docker-Verify] ' + pkg + ' resolved outside standalone: ' + resolved); await import(pathToFileURL(resolved).href); } catch (err) { console.warn('[Docker-Verify] ' + pkg + ' warning:', err.message); } } try { const onnxRuntime = require.resolve('onnxruntime-node'); if (!onnxRuntime.startsWith(standaloneRoot)) console.warn('[Docker-Verify] onnxruntime-node resolved outside standalone: ' + onnxRuntime); await import(pathToFileURL(onnxRuntime).href); } catch (err) { console.warn('[Docker-Verify] onnxruntime-node warning:', err.message); } process.exit(0);" \
  && find /app/.build/next/standalone -type f -name "*.map" -delete \
  && find /app/.build/next/standalone -type f -name "*.ts" ! -name "*.d.ts" -path "*/node_modules/*" -delete
